...
export class DimensionFinder {
        ...
        for (const height of heightRange) {
          ...
          if (percentDifference <= this.tolerancePercent * 100) {
            results.push({
              ...
              percentDifference: formatNumber(percentDifference, 1),
  ...
  getAspectRatio(length, width, height) {
    const lwRatio = formatNumber(length / width, 1);
    const lhRatio = formatNumber(length / height, 1);

    if (length > width * 2) {
      return `Wide (${lwRatio}:1 L:W)`;
    } else if (width > length * 1.5) {
      return `Deep (${formatNumber(width / length, 1)}:1 W:L)`;
