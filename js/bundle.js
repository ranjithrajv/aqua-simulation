// Minimal working aquarium calculator bundle
...
class AquariumApp {
    ...
    updateVolumeDisplays(volumeData) {
        if (this.volumeUnitSystem === VOLUME_UNITS.GALLONS) {
            ...
            DOMHelper.updateVolumeDisplay('waterVolumeInput', 'waterVolumeValue', volumeData.waterGallons);
        } else {
    ...
    updateWaterWeightDisplay(waterGallons) {
        const weightData = this.calculator.calculateWaterWeight(waterGallons, 
    ...
    updateEquipmentRecommendations(dimensions, volumeGallons, surfaceArea) {
        const recommendations = this.equipmentRecommender.getAllRecommendations(
            dimensions.length, dimensions.width, dimensions.height, volumeGallons, surfaceArea
        );
